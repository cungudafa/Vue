    ...
    methods: {
      ...
      submit() {
        this.$refs.loginForm.validate(valid => {//对应el-form的ref属性
          ...
          if (valid) {//表单正确格式填写
            this.$ajax.post('/user/aop/check',this.user).then((res) => {//axios发送post请求，后台验证
            if (res.data) {
              this.$store.dispatch("login", res.data.data).then(() => {//验证结果res.data存储在vuex.vue全局数据控制函数login中
                  this.loading = false;//登录成功，小圆圈停止
                  this.$notify({
                    type: 'success',
                    message: '欢迎你,' + res.data.data.name + '!',
                    duration: 3000
                  })
                  //this.$router.replace('/')
